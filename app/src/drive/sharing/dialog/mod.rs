...
fn access_level_label(access_level: SharingAccessLevel) -> String {
    match access_level {
        SharingAccessLevel::View => t!("drive_extra.sharing.access.can_view").to_string(),
        SharingAccessLevel::Edit => t!("drive_extra.sharing.access.can_edit").to_string(),
        SharingAccessLevel::Full => t!("drive_extra.sharing.access.full").to_string(),
    }
}

fn access_level_name(access_level: SharingAccessLevel) -> String {
    match access_level {
        SharingAccessLevel::View => t!("drive_extra.sharing.access.view_name").to_string(),
        SharingAccessLevel::Edit => t!("drive_extra.sharing.access.edit_name").to_string(),
        SharingAccessLevel::Full => t!("drive_extra.sharing.access.full_name").to_string(),
    }
}
...
impl SharingDialog {
    pub fn new(target: Option<ShareableObject>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let invite_form = EmailInviteForm {
            email_editor: ctx.add_typed_action_view(|ctx| {
                let placeholder = t!("drive_extra.sharing.emails").to_string();
        ...
        self.target
            ...
            .and_then(|target| match target {
                ...
                ShareableObject::Session { .. } => {
                    Some(t!("drive_extra.sharing.session").to_string())
                }
                ShareableObject::AIConversation(_) => {
                    Some(t!("drive_extra.sharing.conversation").to_string())
                }
            })
            .unwrap_or_else(|| t!("drive_extra.sharing.unknown").to_string())
    }
        ...
        if let Some(url) = self.target.as_ref().and_then(|target| target.link(ctx)) {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                let toast = DismissibleToast::default(
                    t!("drive_extra.sharing.copied_link", object = object_name).to_string(),
                );
    ...
    fn reset_guest_menu(&mut self, guest_index: usize, ctx: &mut ViewContext<Self>) {
        if let Some(guest) = self.guest_states.get(guest_index) {
            ...
            self.guest_menu.update(ctx, |menu, ctx| {
                let mut items =
                    vec![
                        MenuItemFields::new(access_level_label(SharingAccessLevel::View))
                ...
                if !is_ai_conversation {
                    items.push(
                        MenuItemFields::new(access_level_label(SharingAccessLevel::Edit))
                            .with_on_select_action(SharingDialogAction::SetGuestAccessLevel(
                ...
                if !is_team_guest || !is_session {
                    ...
                    items.push(
                        MenuItemFields::new(t!("drive_extra.sharing.remove").to_string())
    ...
    fn render_invite_form(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let access_level = SavePosition::new(
            appearance
                ...
                .with_centered_text_label(access_level_label(
        ...
        let mut invite_button = appearance
            ...
            .with_centered_text_label(t!("drive_extra.sharing.invite").to_string())
        ...
        let contents = if validation_state.is_valid() {
            ...
            if !validation_state.duplicate_guests.is_empty() {
                let error_text = t!(
                    "drive_extra.sharing.already_shared",
                    guests = validation_state
                        .duplicate_guests
                        .iter()
                        .format(", ")
                        .to_string()
            ...
            if !validation_state.invalid_emails.is_empty() {
                let error_text = t!(
                    "drive_extra.sharing.invalid_address",
                    addresses = validation_state
                        .invalid_emails
                        .iter()
                        .format(", ")
                        .to_string()
    ...
    fn render_access_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        appearance
            .ui_builder()
            .span(t!("drive_extra.sharing.who_has_access").to_string())
            .with_style(UiComponentStyles {
    ...
    fn render_session_header(
        ...
        let text = appearance
            ...
            .wrappable_text(
                t!(
                    "drive_extra.sharing.live_session_started",
    ...
    fn render_restricted_access_label(
        ...
        let access_name = access_level_name(access_level);
        let text = t!(
            "drive_extra.sharing.restricted_access",
            access = access_name.clone()
        )
        .to_string();
        let access_level_start = text
            .find(&access_name)
            .map(|index| text[..index].chars().count())
            .unwrap_or_default();
        let access_level_end = access_level_start + access_name.chars().count();
    ...
    fn render_owner(&self, appearance: &Appearance, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        let tooltip_text = match owner {
            Subject::Team(_) => t!("drive_extra.sharing.owner_team_tooltip").to_string(),
            Subject::User(_) | Subject::PendingUser { .. } | Subject::AnyoneWithLink(_) => {
                t!("drive_extra.sharing.owner_tooltip").to_string()
            }
        };
        let owner_access_label = render_with_detail_tooltip(
            ...
            appearance
                .ui_builder()
                .span(access_level_label(SharingAccessLevel::Full))
                .with_style(UiComponentStyles {
                    font_color: Some(
                        appearance
                            .theme()
    ...
    fn render_link_sharing_subject(
        ...
        let menu_button_label = match self.link_sharing_state.access_level {
            Some(access_level) => access_level_label(access_level),
            None => t!("drive_extra.sharing.no_access").to_string(),
        };
    ...
    fn reset_link_sharing_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let mut items = vec![
            MenuItemFields::new(t!("drive_extra.sharing.only_people_invited").to_string())
                .with_on_select_action(SharingDialogAction::SetLinkPermissions(None))
                .with_icon(Icon::Lock)
                .with_disabled(inherited_access)
                .into_item(),
            MenuItem::Separator,
            MenuItemFields::new(t!("drive_extra.sharing.link_section").to_string())
    ...
    fn reset_team_sharing_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let items = [
            MenuItemFields::new(t!("drive_extra.sharing.only_invited_teammates").to_string())
                .with_on_select_action(SharingDialogAction::SetTeamPermissions(None))
                .with_icon(Icon::Lock)
                .with_disabled(inherited_access)
                .into_item(),
            MenuItem::Separator,
            MenuItemFields::new(t!("drive_extra.sharing.teammates_with_link").to_string())
                .with_no_interaction_on_hover()
    ...
    fn render_guest(
        ...
        let mut access_level_button = appearance
            ...
            .with_centered_text_label(access_level_label(guest.current_access_level))
            .with_style(UiComponentStyles {
                padding: Some(Coords::default()),
                ..Default::default()
            });
    ...
    fn render_subject(
        ...
        let name_text = subject
            .name(app)
            .unwrap_or_else(|| Cow::Owned(t!("drive_extra.sharing.unknown").to_string()));
    ...
    fn download_qr_code(&self, ctx: &mut ViewContext<Self>) {
        let Some(url) = self.target_link(ctx) else {
            self.show_ephemeral_toast(
                DismissibleToast::error(t!("drive_extra.sharing.qr.download_failed").to_string()),
                ctx,
            );
            return;
        };
            ...
            Err(_) => {
                self.show_ephemeral_toast(
                    DismissibleToast::error(
                        t!("drive_extra.sharing.qr.download_failed").to_string(),
                    ),
                    ctx,
                );
                return;
    ...
    fn handle_qr_write_result(&self, result: std::io::Result<()>, ctx: &mut ViewContext<Self>) {
        match result {
            Ok(()) => self.show_ephemeral_toast(
                DismissibleToast::success(t!("drive_extra.sharing.qr.downloaded").to_string()),
                ctx,
            ),
    ...
    fn render_qr_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let title = appearance
            .ui_builder()
            .span(t!("drive_extra.sharing.qr.title").to_string())
            .with_style(UiComponentStyles {
                font_color: Some(foreground),
                font_size: Some(style::HEADER_TEXT_SIZE),
                ..Default::default()
    ...
    fn render_qr_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let qr_contents = self
                ...
                let action_row = Flex::row()
                    ...
                    .with_children([
                        self.render_footer_icon_button(
                            ...
                            t!("drive_extra.sharing.qr.copy_link").to_string(),
                        ...
                        Container::new(self.render_footer_icon_button(
                            ...
                            t!("drive_extra.sharing.qr.download").to_string(),
            ...
            .unwrap_or_else(|| {
                appearance
                    .ui_builder()
                    .paragraph(t!("drive_extra.sharing.qr.create_failed").to_string())
                    .with_style(UiComponentStyles {
                        font_color: Some(style::acl_secondary_text_color(appearance)),
                        ..Default::default()
                    })
    ...
    fn render_object_link(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let qr_button = matches!(self.target, Some(ShareableObject::Session { .. })).then(|| {
            self.render_footer_icon_button(
                ...
                t!("drive_extra.sharing.qr.show").to_string(),
