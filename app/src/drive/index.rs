...
fn localized_object_type_plural(object_type: ObjectType) -> String {
    match object_type {
        ObjectType::Notebook => t!("drive_extra.index.usage.notebooks").to_string(),
        ObjectType::Workflow => t!("drive_extra.index.usage.workflows").to_string(),
        ObjectType::Folder => t!("drive_extra.index.usage.folders").to_string(),
        ObjectType::GenericStringObject(GenericStringObjectFormat::Json(json_type)) => {
            match json_type {
                JsonObjectType::Preference => t!("drive_extra.index.usage.preferences").to_string(),
                JsonObjectType::EnvVarCollection => {
                    t!("drive_extra.index.usage.environment_variables").to_string()
                }
                JsonObjectType::WorkflowEnum => {
                    t!("drive_extra.index.usage.workflow_enums").to_string()
                }
                JsonObjectType::AIFact => t!("drive_extra.index.usage.rules").to_string(),
                JsonObjectType::MCPServer => t!("drive_extra.index.usage.mcp_servers").to_string(),
                JsonObjectType::AIExecutionProfile => {
                    t!("drive_extra.index.usage.ai_execution_profiles").to_string()
                }
                JsonObjectType::TemplatableMCPServer => {
                    t!("drive_extra.index.usage.mcp_server_templates").to_string()
                }
                JsonObjectType::CloudEnvironment => {
                    t!("drive_extra.index.usage.cloud_environments").to_string()
                }
                JsonObjectType::ScheduledAmbientAgent => {
                    t!("drive_extra.index.usage.scheduled_agents").to_string()
                }
                JsonObjectType::CloudAgentConfig => {
                    t!("drive_extra.index.usage.cloud_agent_configs").to_string()
                }
            }
        }
    }
...
impl DriveIndex {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let title_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(t!("drive_extra.common.untitled"), ctx);
            editor
        });
    ...
    fn render_trash_section_header(
        ...
        let mut empty_trash_button = appearance
            ...
            .with_text_label(t!("drive_extra.index.empty_trash").to_string());
    ...
    fn render_section_header(
        ...
        let rendered_header = match (&self.index_variant, section) {
            ...
            (DriveIndexVariant::MainIndex, DriveIndexSection::CreateATeam) => {
                if self.is_online(app) {
                    Some(self.render_team_section_header(
                        t!("drive_extra.index.share_with_teammates").to_string(),
                        appearance,
                    ))
                } else {
                    None
            ...
            (DriveIndexVariant::MainIndex, DriveIndexSection::JoinTeam) => {
                if self.is_online(app) {
                    let join_teams_text = t!(
                        "drive_extra.index.collaborate_count",
                        count = UserWorkspaces::handle(app)
                            .as_ref(app)
                            .total_teammates_in_joinable_teams()
                    )
                    .to_string();
    ...
    fn render_trash_row(&self, appearance: &Appearance, _: &AppContext) -> Box<dyn Element> {
        ...
        let title = Container::new(
            appearance
                .ui_builder()
                .wrappable_text(t!("drive_extra.index.trash_upper").to_string(), false)
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(SECTION_HEADER_FONT_SIZE),
                    font_weight: Some(Weight::Normal),
    ...
    fn render_team_space_zero_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        let hint_text = t!("drive_extra.index.team_drop_hint").to_string();
        ...
        let workflow_label = t!("drive_extra.index.object_labels.workflow").to_string();
        let notebook_label = t!("drive_extra.index.object_labels.notebook").to_string();
        let zero_state_contents = Flex::column().with_children([
            zero_state_info,
            self.render_team_zero_state_hint(Icon::Workflow, workflow_label.clone(), appearance),
            self.render_team_zero_state_hint(Icon::Workflow, workflow_label, appearance),
            self.render_team_zero_state_hint(Icon::Notebook, notebook_label.clone(), appearance),
            self.render_team_zero_state_hint(Icon::Notebook, notebook_label, appearance),
        ]);
    ...
    fn render_create_team_section(
        ...
        let button_text = t!("drive_extra.index.create_team").to_string();
    ...
    fn render_join_discoverable_team_section(
        ...
        let text = if UserWorkspaces::as_ref(app).num_joinable_teams() > 1 {
            t!("drive_extra.index.view_teams_to_join").to_string()
        } else {
            t!("drive_extra.index.view_team_to_join").to_string()
        };
        ...
        let or_text = Container::new(
            Text::new_inline(
                t!("drive_extra.index.or").to_string(),
            ...
            Flex::row()
                ...
                .with_child(
                    Shrinkable::new(
                        1.,
                        Text::new_inline(
                            t!("drive_extra.index.offline").to_string(),
                            appearance.ui_font_family(),
    ...
    fn render_title(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let text = Container::new(
            appearance
                .ui_builder()
                .span(t!("drive_extra.index.title").to_string())
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
    ...
    fn render_trash_title(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = Container::new(
            appearance
                .ui_builder()
                .span(t!("drive_extra.index.trash").to_string())
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(TITLE_FONT_SIZE),
                    font_weight: Some(Weight::Semibold),
                ...
                .with_child(
                    Shrinkable::new(
                        ...
                        appearance
                            .ui_builder()
                            .wrappable_text(
                                t!("drive_extra.index.trash_retention").to_string(),
                                true,
                            )
                            .with_style(UiComponentStyles {
                                font_family_id: Some(appearance.ui_font_family()),
            ...
            |mouse_state| {
                ...
                if mouse_state.is_hovered() {
                    let tooltip = appearance
                        .ui_builder()
                        .tool_tip(t!("drive_extra.index.syncing").to_string());
        ...
        let hoverable = Hoverable::new(
            ...
            |mouse_state| {
                if mouse_state.is_hovered() {
                    let tooltip = appearance
                        .ui_builder()
                        .tool_tip(t!("drive_extra.index.sort_by").to_string());
    ...
    fn render_retry_button(&self, appearance: &Appearance) -> Box<dyn warpui::Element> {
        ...
        .with_tooltip(move || {
            ui_builder
                .tool_tip(t!("drive_extra.index.retry_sync").to_string())
                .build()
                .finish()
        })
        .build()
    ...
    fn toggle_new_assets_menu(&mut self, space: &Space, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(state) = state {
            ...
            if is_online {
                menu_items.push(
                    MenuItemFields::new(t!("drive_extra.index.object_labels.folder").to_string())
            ...
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.index.object_labels.workflow").to_string())
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Workflow,
                        *space,
                        None,
            ...
            if FeatureFlag::AgentModeWorkflows.is_enabled() {
                menu_items.push(
                    MenuItemFields::new(t!("drive_extra.index.object_labels.prompt").to_string())
                        .with_on_select_action(DriveIndexAction::create_object(
                            DriveObjectType::AgentModeWorkflow,
                            *space,
                            None,
            ...
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.index.object_labels.notebook").to_string())
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Notebook {
                            is_ai_document: false,
                        },
            ...
            menu_items.push(
                MenuItemFields::new(
                    t!("drive_extra.index.object_labels.environment_variables").to_string(),
                )
                .with_on_select_action(DriveIndexAction::create_object(
                    DriveObjectType::EnvVarCollection,
                    *space,
            ...
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.index.import").to_string())
    ...
    fn render_personal_limit_status(
        ...
        let header = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    t!("drive_extra.index.title").to_string(),
                    appearance.ui_font_family(),
                    14.,
                )
        ...
        let personal_object_limit_description =
            t!("drive_extra.index.signup_description").to_string();
        ...
        let button = appearance
            ...
            .with_centered_text_label(t!("drive_extra.index.sign_up").to_string())
            .build()
    ...
    fn render_personal_object_limit_row(
        ...
        let name = match object_type {
            DriveObjectType::Notebook { .. } => t!("drive_extra.index.usage.notebooks").to_string(),
            DriveObjectType::Workflow => t!("drive_extra.index.usage.workflows").to_string(),
            DriveObjectType::EnvVarCollection => {
                t!("drive_extra.index.usage.environment_variables").to_string()
            }
            DriveObjectType::Folder => t!("drive_extra.index.usage.folders").to_string(),
            DriveObjectType::AgentModeWorkflow => {
                t!("drive_extra.index.usage.agent_workflows").to_string()
            }
            DriveObjectType::AIFact => t!("drive_extra.index.usage.ai_fact").to_string(),
            DriveObjectType::AIFactCollection => t!("drive_extra.index.usage.rules").to_string(),
            DriveObjectType::MCPServer => t!("drive_extra.index.usage.mcp_server").to_string(),
            DriveObjectType::MCPServerCollection => {
                t!("drive_extra.index.usage.mcp_servers").to_string()
            }
        };
    ...
    fn render_shared_object_limit_hit_banner(
        ...
        let object_type = localized_object_type_plural(object_type);
        let banner_line_1 = t!(
            "drive_extra.index.shared_limit_reached",
            object_type = object_type
        )
        .to_string();
        let upgrade_message = t!("drive_extra.index.shared_limit_upgrade");
        let body = Container::new(
            appearance
                .ui_builder()
                .wrappable_text(format!("{banner_line_1} {upgrade_message}"), true)
                .with_highlights(
        ...
        let button = appearance
            ...
            .with_centered_text_label(t!("drive_extra.index.compare_plans").to_string())
            .with_style(UiComponentStyles {
                font_size: Some(14.),
                font_weight: Some(Weight::Light),
                padding: Some(Coords {
    ...
    fn render_payment_issue_banner(
        ...
        let banner_line_2 = if has_admin_permissions && is_on_stripe_paid_plan {
            t!("drive_extra.index.payment_admin").to_string()
        } else if has_admin_permissions && !is_on_stripe_paid_plan {
            t!("drive_extra.index.payment_enterprise_admin").to_string()
        } else {
            t!("drive_extra.index.payment_non_admin").to_string()
        };
        let banner_line_1 = t!("drive_extra.index.payment_issue").to_string();
        ...
        if has_admin_permissions && is_on_stripe_paid_plan {
            let button = appearance
                ...
                .with_centered_text_label(t!("drive_extra.index.manage_billing").to_string())
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
                    font_weight: Some(Weight::Light),
                    padding: Some(Coords {
    ...
    fn index_menu_items(
        ...
        if let CloudObjectTypeAndId::Folder(folder_id) = cloud_object_type_and_id {
            if let SyncId::ServerId(_) = folder_id {
                if self.is_online(app) {
                    if !FeatureFlag::SharedWithMe.is_enabled() || editability.can_edit() {
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.new_object.folder").to_string(),
                            )
                            .with_on_select_action(DriveIndexAction::create_object(
                                DriveObjectType::Folder,
                                *space,
                        ...
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.new_object.workflow").to_string(),
                            )
                            .with_on_select_action(DriveIndexAction::create_object(
                                DriveObjectType::Workflow,
                                *space,
                        ...
                        if FeatureFlag::AgentModeWorkflows.is_enabled() {
                            menu_items.push(
                                MenuItemFields::new(
                                    t!("drive_extra.index.new_object.prompt").to_string(),
                                )
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::AgentModeWorkflow,
                                    *space,
                        ...
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.new_object.notebook").to_string(),
                            )
                            .with_on_select_action(DriveIndexAction::create_object(
                                DriveObjectType::Notebook {
                                    is_ai_document: false,
                        ...
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.new_object.environment_variables")
                                    .to_string(),
                            )
                            .with_on_select_action(DriveIndexAction::create_object(
                                DriveObjectType::EnvVarCollection,
                    ...
                    if !FeatureFlag::SharedWithMe.is_enabled() || editability.can_edit() {
                        menu_items.push(
                            MenuItemFields::new(t!("drive_extra.common.rename").to_string())
                ...
                if let Some(object) = object {
                    if let Some(object_link) = object.object_link() {
                        menu_items.push(
                            MenuItemFields::new(t!("drive_extra.common.copy_link").to_string())
                        ...
                        if editability.can_edit() {
                            menu_items.push(
                                MenuItemFields::new(t!("drive_extra.common.share").to_string())
                ...
                menu_items.push(
                    MenuItemFields::new(t!("drive_extra.index.menu.collapse_all").to_string())
                ...
                if let Some(object) = object {
                    if FeatureFlag::SharedWithMe.is_enabled() && object.can_leave(app) {
                        menu_items.push(
                            MenuItemFields::new(t!("drive_extra.index.remove").to_string())
            ...
            if let Some(object) = object {
                if self.is_online(app) && object.metadata().is_errored() {
                    menu_items.push(
                        MenuItemFields::new(t!("drive_extra.common.retry").to_string())
                    ...
                    if let Some(server_id) = cloud_object_type_and_id.server_id() {
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.menu.revert_to_server").to_string(),
                            )
                ...
                if self.edit_object_enabled(cloud_object_type_and_id, app) {
                        ...
                        >(object)
                    {
                        if let Some(ai_document_id) = notebook.model().ai_document_id {
                            menu_items.push(
                                MenuItemFields::new(
                                    t!("drive_extra.index.menu.attach_to_active_session")
                                        .to_string(),
                                )
                ...
                match object.object_type() {
                    ObjectType::Workflow => {
                        ...
                        let label = if workflow.model().data.is_agent_mode_workflow() {
                            t!("drive_extra.index.menu.copy_prompt").to_string()
                        } else {
                            t!("drive_extra.index.menu.copy_workflow_text").to_string()
                        };
                        menu_items.push(
                        ...
                        if workflow.model().data.is_agent_mode_workflow() {
                            menu_items.push(
                                MenuItemFields::new(
                                    t!("drive_extra.index.menu.copy_id").to_string(),
                                )
                    ...
                    ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                        ...
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.menu.copy_variables").to_string(),
                            )
                            .with_on_select_action(DriveIndexAction::CopyObjectToClipboard(
                                *cloud_object_type_and_id,
                            ))
                        ...
                        menu_items.push(
                            MenuItemFields::new(
                                t!("drive_extra.index.menu.load_in_subshell").to_string(),
                            )
                            .with_on_select_action(
                            ...
                            if let DriveIndexSection::Space(space) = section {
                                match space {
                                    ...
                                    Space::Team { .. } => Some(
                                        MenuItemFields::new(
                                            t!(
                                                "drive_extra.index.menu.move_to",
                                                name = space.name(app)
                                            )
                                            .to_string(),
                                        )
            ...
            if let Some(object) = object {
                match object.object_type() {
                    ...
                    | ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                                ...
                                == UserAppInstallStatus::Detected
                        {
                            if let Some(object_link) = object.object_link() {
                                if let Ok(url) = Url::parse(&object_link) {
                                    menu_items.push(
                                        MenuItemFields::new(
                                            t!("drive_extra.index.menu.open_on_desktop")
                                                .to_string(),
                                        )
                                        .with_on_select_action(
                        ...
                        {
                            menu_items.push(
                                MenuItemFields::new(t!("drive_extra.common.duplicate").to_string())
                ...
                if object.can_export() {
                    menu_items.push(
                        MenuItemFields::new(t!("drive_extra.common.export").to_string())
        ...
        {
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.common.trash").to_string())
    ...
    fn pane_menu_item(
        ...
        if (FeatureFlag::SharedWithMe.is_enabled() && !editability.can_edit()) || prefer_open {
            MenuItemFields::new(t!("drive_extra.common.open").to_string()).with_icon(Icon::Eye)
        } else {
            MenuItemFields::new(t!("drive_extra.common.edit").to_string()).with_icon(Icon::Rename)
        }
    }
    ...
    fn trash_menu_items(
        ...
        if self.online_only_operation_allowed(cloud_object_type_and_id, app) {
            if !FeatureFlag::SharedWithMe.is_enabled() || access_level.can_trash() {
                menu_items.push(
                    MenuItemFields::new(t!("drive_extra.common.restore").to_string())
            ...
            if !FeatureFlag::SharedWithMe.is_enabled() || access_level.can_delete() {
                menu_items.push(
                    MenuItemFields::new(t!("drive_extra.index.menu.delete_forever").to_string())
