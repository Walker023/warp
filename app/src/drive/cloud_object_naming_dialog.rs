...
impl CloudObjectNamingDialog {
    ...
    fn render_text_header(
        ...
        let title = match object_type {
            DriveObjectType::Notebook { .. } => {
                t!("drive_extra.dialogs.naming.notebook_name").to_string()
            }
            DriveObjectType::Folder => t!("drive_extra.dialogs.naming.folder_name").to_string(),
            DriveObjectType::EnvVarCollection => {
                t!("drive_extra.dialogs.naming.collection_name").to_string()
            }
    ...
    fn render_action_buttons(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let primary_button_text = match self.is_rename {
            true => t!("drive_extra.common.rename").to_string(),
            false => t!("drive_extra.common.create").to_string(),
        };
