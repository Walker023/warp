...
impl SessionType {
    ...
    pub(crate) fn pill_label(&self) -> String {
        match self {
            SessionType::Terminal => t!("tab_configs_ui.session.terminal").to_string(),
            SessionType::Oz => t!("tab_configs_ui.session.built_in_agent").to_string(),
...
fn config_name(directory: &Path, enable_worktree: bool) -> String {
    let repo = directory
        ...
        .map(str::to_owned)
        .unwrap_or_else(|| t!("tab_configs_ui.session.untitled").to_string());
    if enable_worktree {
        t!(
            "tab_configs_ui.session.worktree_config_name",
            repo = repo.as_str()
        )
        .to_string()
    } else {
        t!(
            "tab_configs_ui.session.new_tab_config_name",
            repo = repo.as_str()
        )
        .to_string()
    }
    ...
    if enable_worktree {
        if autogenerate_worktree_branch_name {
            ...
            params.insert(
                ...
                TabConfigParam {
                    description: Some(
                        t!("tab_configs_ui.session.worktree_branch_description").to_string(),
                    ),
...
pub fn tab_config_from_pane_snapshot(
    ...
    TabConfig {
        name: t!("tab_configs_ui.session.default_config_name").to_string(),
        title: custom_title,
        color,
        panes,
        params: HashMap::new(),
