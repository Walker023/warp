...
impl FileTreeView {
    ...
    fn context_menu_items(
        ...
        if is_remote {
            ...
            match item {
                FileTreeItem::File { .. } => {
                    ...
                    if !is_file_content_binary(&path_local) {
                        items.extend([
                            MenuItemFields::new(
                                t!("code_editor_extra.file_tree.open_in_new_pane").to_string(),
                            )
                            .with_on_select_action(FileTreeAction::OpenInNewPane { id: id.clone() })
                            .into_item(),
                            MenuItemFields::new(
                                t!("code_editor_extra.file_tree.open_in_new_tab").to_string(),
                            )
                        ...
                        items.push(
                            MenuItemFields::new(
                                t!("code_editor_extra.file_tree.open_file").to_string(),
                            )
                ...
                FileTreeItem::DirectoryHeader { .. } => {
                    items.push(
                        MenuItemFields::new(t!("code_editor_extra.file_tree.new_file").to_string())
                    ...
                    if self.has_terminal_session {
                        items.push(
                            MenuItemFields::new(
                                t!("code_editor_extra.file_tree.cd_to_directory").to_string(),
                            )
            ...
            let open_text = if cfg!(target_os = "macos") {
                t!("code_editor_extra.file_tree.reveal_in_finder").to_string()
            } else if cfg!(target_os = "windows") {
                t!("code_editor_extra.file_tree.reveal_in_explorer").to_string()
            } else {
                t!("code_editor_extra.file_tree.reveal_in_file_manager").to_string()
            };
            items.push(
            ...
            if !is_repo_root_dir {
                items.push(
                    MenuItemFields::new(t!("code_editor_extra.file_tree.rename").to_string())
                        .with_on_select_action(FileTreeAction::Rename { id: id.clone() })
                        .into_item(),
                );
                items.push(
                    MenuItemFields::new(t!("code_editor_extra.file_tree.delete").to_string())
        ...
        if self.has_terminal_session {
            ...
            items.push(
                MenuItemFields::new(
                    t!("code_editor_extra.file_tree.attach_as_context").to_string(),
                )
        ...
        items.extend([
            MenuItemFields::new(t!("code_editor_extra.file_tree.copy_path").to_string())
                .with_on_select_action(FileTreeAction::CopyPath { id: id.clone() })
                .into_item(),
            MenuItemFields::new(t!("code_editor_extra.file_tree.copy_relative_path").to_string())
    ...
    fn render_error_state(&self, text: String, app: &AppContext) -> Box<dyn Element> {
        ...
        let main_column = Flex::column()
            ...
            .with_child(
                Text::new(
                    t!("code_editor_extra.file_tree.unavailable_title").to_string(),
                    appearance.ui_font_family(),
...
impl View for FileTreeView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        self.render_error_state(
            t!("code_editor_extra.file_tree.remote_unavailable").to_string(),
            app,
        )
    }
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        if matches!(self.enablement, CodingPanelEnablementState::Disabled) {
            return self.render_error_state(
                t!("code_editor_extra.file_tree.disabled_unavailable").to_string(),
                app,
            );
        }
        ...
        if self.displayed_directories.is_empty() {
            ...
            if matches!(
                ...
                return self.render_error_state(
                    t!("code_editor_extra.file_tree.wsl_unavailable").to_string(),
                    app,
                );
            }
