    ...
    pub fn from_context(command_context: CommandContext) -> CommandRenderInfo {
        match command_context {
            CommandContext::RunningCommand { running_command } => CommandRenderInfo {
                ...
                hint_text: t!("workspace_search_ui.search.navigation.running").to_string(),
            ...
            CommandContext::LastRunCommand {
                ...
                hint_text: match mins_since_completion {
                    Some(mins) if mins >= 60 => {
                        t!("workspace_search_ui.search.navigation.completed_over_hour_ago")
                            .to_string()
                    }
                    Some(mins) if mins == 1 => t!(
                        "workspace_search_ui.search.navigation.completed_minute_ago",
                        count = mins
                    )
                    .to_string(),
                    Some(mins) => t!(
                        "workspace_search_ui.search.navigation.completed_minutes_ago",
                        count = mins
                    )
                    .to_string(),
                    None => t!("workspace_search_ui.search.navigation.no_timestamp").to_string(),
                },
            },
            ...
            CommandContext::LastRunAIBlock { prompt } => CommandRenderInfo {
                command_text: Some(prompt),
                hint_text: t!("workspace_search_ui.search.navigation.completed").to_string(),
                row_spacing: styles::NAVIGATION_PALETTE_COMMAND_ROW_SPACING,
                hint_margin: styles::NAVIGATION_PALETTE_COMMAND_HINT_MARGIN,
            },
            CommandContext::None => CommandRenderInfo {
                command_text: Some(String::new()),
                hint_text: t!("workspace_search_ui.search.navigation.empty_session").to_string(),
                row_spacing: 0.,
                hint_margin: 0.,
            },
        }
