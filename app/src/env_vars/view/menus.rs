...
impl EnvVarCollectionView {
    pub(super) fn initialize_menus(ctx: &mut ViewContext<Self>) -> Menus {
        ...
        let clear_secret_item = Self::item(
            t!("env_vars_ui.menu.clear_secret"),
    ...
    pub(super) fn initialize_pane_context_menu(
        ...
        let toggle_maximize_pane = Self::item(
            if is_maximized {
                t!("menu.minimize_pane")
            } else {
                t!("menu.maximize_pane")
            },
    ...
    pub(super) fn overflow_menu_items(
        ...
        if let Some(link) = self.env_var_collection_link(ctx) {
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.common.copy_link"))
        ...
        if space != Some(Space::Shared) {
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.common.duplicate"))
        ...
        {
            menu_items.push(
                MenuItemFields::new(t!("drive_extra.common.trash"))
        ...
        menu_items.push(
            MenuItemFields::new(t!("drive_extra.common.export"))
