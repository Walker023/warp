...
impl LaunchConfigSaveModal {
    ...
    fn render_save_config_button(
        ...
        self.save_modal_button(
            appearance,
            t!("launch_configs_ui.save_modal.save_configuration").to_string(),
    ...
    fn render_open_file_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        self.save_modal_button(
            appearance,
            t!("launch_configs_ui.save_modal.open_yaml_file").to_string(),
        ...
        let header = Flex::row()
            .with_child(
                Shrinkable::new(
                    ...
                    Align::new(
                        Text::new_inline(
                            t!("launch_configs_ui.save_modal.title").to_string(),
        ...
        let link_to_docs = Container::new(
            ConstrainedBox::new(
                appearance
                    .ui_builder()
                    .link(
                        t!("launch_configs_ui.save_modal.documentation").to_string(),
                        Some(
        ...
        let info = match &self.save_state {
                    ...
                    self.render_formatted_text_line(
                        ...
                        vec![
                            FormattedTextFragment::plain_text(t!(
                                "launch_configs_ui.save_modal.saved_prefix"
                            )),
                            FormattedTextFragment::inline_code(
                                self.file_name.clone().unwrap_or_default(),
                            ),
                            FormattedTextFragment::plain_text(t!(
                                "launch_configs_ui.save_modal.sentence_end"
                            )),
                        ],
                    )
                ...
                self.render_text_block(
                    ...
                    match failure_type {
                        FailureType::FileAlreadyExists => {
                            t!("launch_configs_ui.save_modal.file_exists").to_string()
                        }
                        FailureType::Other => {
                            t!("launch_configs_ui.save_modal.save_failed").to_string()
                        }
                    },
                )
                .with_padding_bottom(24.)
            ...
            SaveState::NotSaved => {
                let text = if self.open_modal_keybinding_str.is_empty() {
                    t!("launch_configs_ui.save_modal.description").to_string()
                } else {
                    t!(
                        "launch_configs_ui.save_modal.description_with_keybinding",
                ...
                header
                    ...
                    .with_child(
                        self.render_formatted_text_line(
                            ...
                            vec![
                                FormattedTextFragment::plain_text(t!(
                                    "launch_configs_ui.save_modal.yaml_location_prefix"
                                )),
...
impl View for LaunchConfigSaveModal {
    ...
    fn accessibility_contents(&self, _ctx: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            t!("launch_configs_ui.save_modal.accessibility_title"),
            t!("launch_configs_ui.save_modal.accessibility_description"),
