...
fn localized_agent_source_name(source: &AgentSource) -> String {
    match source {
        AgentSource::Linear => t!("ai_ui.agent_management.source.linear").to_string(),
        AgentSource::AgentWebhook => t!("ai_ui.agent_management.source.api").to_string(),
        AgentSource::Slack => t!("ai_ui.agent_management.source.slack").to_string(),
        AgentSource::Cli => t!("ai_ui.agent_management.source.cli").to_string(),
        AgentSource::ScheduledAgent => t!("ai_ui.agent_management.source.scheduled").to_string(),
        AgentSource::Interactive | AgentSource::CloudMode => {
            t!("ai_ui.agent_management.source.warp_app").to_string()
        }
        AgentSource::WebApp => t!("ai_ui.agent_management.source.oz_web").to_string(),
        AgentSource::GitHubAction => t!("ai_ui.agent_management.source.github_action").to_string(),
    }
}
...
impl AgentManagementView {
    pub fn new(
        ...
        let all_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(t!("ai_ui.agent_management.all").to_string(), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(t!("ai_ui.agent_management.all_tasks_tooltip").to_string())
                .on_click(|ctx| {
        ...
        let personal_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                t!("ai_ui.agent_management.personal").to_string(),
                NakedTheme,
            )
            .with_size(ButtonSize::Small)
            .with_tooltip(t!("ai_ui.agent_management.personal_tasks_tooltip").to_string())
            .on_click(|ctx| {
                ctx.dispatch_typed_action(AgentManagementViewAction::SetOwnerFilter(
        ...
        let setup_guide_button = CompactibleActionButton::new(
            t!("ai_ui.agent_management.get_started").to_string(),
            None,
            ButtonSize::Small,
        ...
        let view_agents_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                t!("ai_ui.agent_management.view_agents").to_string(),
                NakedTheme,
            )
            .with_size(ButtonSize::Small)
        ...
        let no_filter_results_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(
                t!("ai_ui.agent_management.clear_filters").to_string(),
                SecondaryTheme,
            )
            .with_size(ButtonSize::Small)
        ...
        let clear_all_filters_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(
                t!("ai_ui.agent_management.clear_all").to_string(),
                NakedTheme,
            )
            .with_icon(Icon::X)
            .with_size(ButtonSize::Small)
        ...
        let new_agent_button = CompactibleActionButton::new(
            t!("ai_ui.agent_management.new_agent").to_string(),
            None,
            ButtonSize::Small,
    ...
    fn create_status_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.status").to_string(),
            ctx,
        );
    ...
    fn create_source_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.source").to_string(),
            ctx,
        );
    ...
    fn build_source_dropdown_items() -> Vec<MenuItem<DropdownAction>> {
        ...
        let mut items = vec![MenuItem::Item(
            MenuItemFields::new(t!("ai_ui.agent_management.all").to_string())
        ...
        for source in sources {
            let label = localized_agent_source_name(&source);
    ...
    fn create_created_on_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.created_on").to_string(),
            ctx,
        );
        ...
        let items = vec![
            ...
            MenuItem::Item(
                MenuItemFields::new(
                    t!("ai_ui.agent_management.created_on.last_24_hours").to_string(),
                )
                .with_on_select_action(DropdownAction::select_action_and_close(
            ...
            MenuItem::Item(
                MenuItemFields::new(
                    t!("ai_ui.agent_management.created_on.past_3_days").to_string(),
                )
                .with_on_select_action(DropdownAction::select_action_and_close(
            ...
            MenuItem::Item(
                MenuItemFields::new(t!("ai_ui.agent_management.created_on.last_week").to_string())
                    .with_on_select_action(DropdownAction::select_action_and_close(
    ...
    fn create_artifact_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.has_artifact").to_string(),
            ctx,
        );
        ...
        let items = vec![
            ...
            MenuItem::Item(
                MenuItemFields::new(t!("ai_ui.agent_management.artifact.pull_request").to_string())
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::PullRequest),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(t!("ai_ui.agent_management.artifact.plan").to_string())
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::Plan),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(t!("ai_ui.agent_management.artifact.screenshot").to_string())
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::Screenshot),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(t!("ai_ui.agent_management.artifact.file").to_string())
                    .with_on_select_action(DropdownAction::select_action_and_close(
    ...
    fn create_harness_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.harness").to_string(),
            ctx,
        );
    ...
    fn create_environment_dropdown(
        ...
        Self::setup_searchable_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.environment").to_string(),
            ctx,
        );
        ...
        dropdown.set_menu_header_text_override(|text| {
            ...
            t!(
                "ai_ui.agent_management.filter_label",
                label = t!("ai_ui.agent_management.filters.environment"),
                value = &value
            )
            .to_string()
        });
    ...
    fn create_creator_dropdown(
        ...
        Self::setup_searchable_filter_menu(
            &mut dropdown,
            t!("ai_ui.agent_management.filters.created_by").to_string(),
            ctx,
        );
        dropdown
    }
    ...
    fn update_environment_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let selected_name = match &self.filters.environment {
            EnvironmentFilter::All => Some(t!("ai_ui.agent_management.all").to_string()),
            EnvironmentFilter::NoEnvironment => Some(t!("common.none").to_string()),
        ...
        self.environment_dropdown.update(ctx, |dropdown, ctx| {
            ...
            items.push(MenuItem::Item(
                MenuItemFields::new(t!("common.none").to_string()).with_on_select_action(
                    DropdownAction::select_action_and_close(
    ...
    fn update_creator_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let creator_filter_name = match &self.filters.creator {
            CreatorFilter::All => t!("ai_ui.agent_management.all").to_string(),
            ...
            ArtifactButtonsRowEvent::CopyBranch { branch } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::default(
                        t!("ai_ui.agent_management.copied_branch_name").to_string(),
                    );
    ...
    fn render_session_status_label(
        ...
        let (label_text, tooltip_text_opt) = match session_status {
            SessionStatus::Expired => (
                t!("ai_ui.agent_management.session.expired").to_string(),
                Some(t!("ai_ui.agent_management.session.expired_tooltip").to_string()),
            ),
            SessionStatus::Unavailable => (
                t!("ai_ui.agent_management.session.unavailable").to_string(),
                None,
            ),
    ...
    fn render_header_row(
        ...
        let creator_name = entry
            ...
            .name
            .clone()
            .unwrap_or_else(|| t!("ai_ui.agent_management.unknown").to_string());
    ...
    fn render_metadata_row(
        ...
        if let Some(source) = &entry.display.source {
            metadata_parts.push(
                t!(
                    "ai_ui.agent_management.metadata.source",
                    name = localized_agent_source_name(source)
                )
                .to_string(),
            );
        }
        ...
        if availability.should_show_harness_selector() {
            if let Some(harness) = entry.display.harness {
                metadata_parts.push(
                    t!(
                        "ai_ui.agent_management.metadata.harness",
                        name = availability.display_name_for(harness)
                    )
                    .to_string(),
                );
            }
            ...
            if !same_as_creator {
                if let Some(name) = executor.name.as_deref().or(executor.uid.as_deref()) {
                        ...
                        .is_some_and(|pt| pt.is_service_account())
                    {
                        ...
                        t!("ai_ui.agent_management.metadata.executor")
                    };
                    metadata_parts.push(
                        t!(
                            "ai_ui.agent_management.metadata.named_value",
                            label = label,
                            name = name
                        )
                        .to_string(),
        ...
        if let Some(run_time) = &entry.display.run_time {
            metadata_parts
                .push(t!("ai_ui.agent_management.metadata.run_time", time = run_time).to_string());
        }

        if let Some(usage) = entry.display.request_usage.map(format_credits) {
            metadata_parts.push(
                t!(
                    "ai_ui.agent_management.metadata.credits_used",
                    usage = usage
                )
                .to_string(),
            );
    ...
    fn render_task_list_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let build_header = |use_expanded: bool| {
            let title = Text::new_inline(
                t!("ai_ui.agent_management.runs").to_string(),
        ...
        Hoverable::new(self.loading_icon_mouse_state.clone(), move |mouse_state| {
            ...
            if mouse_state.is_hovered() {
                let tooltip = ui_builder
                    .tool_tip(t!("ai_ui.agent_management.loading_cloud_runs").to_string())
                    .build()
                    .finish();
                stack.add_positioned_overlay_child(
                    tooltip,
    ...
    fn render_loading_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let loading_row = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    t!("ai_ui.agent_management.loading_agents").to_string(),
                    appearance.ui_font_family(),
    ...
    fn render_no_results_view(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let text = Text::new_inline(
            t!("ai_ui.agent_management.no_filter_results").to_string(),
            appearance.ui_font_family(),
