...
impl EnvironmentDisplayData {
    ...
    fn format_timestamp_text(&self) -> String {
        let last_edited_part = self.last_edited_ts.map(|ts| {
            t!(
                "settings.environments.last_edited",
        ...
        let last_used_part = match self.last_used_ts {
            Some(ts) => t!(
                "settings.environments.last_used",
                time = format_approx_duration_from_now_utc(ts.utc())
            )
            .to_string(),
            None => t!("settings.environments.last_used_never").to_string(),
        };
...
impl EnvironmentsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let pane_configuration = ctx.add_model(|_| {
            crate::pane_group::pane::PaneConfiguration::new(
                t!("settings.environments.title").to_string(),
            )
        });
    ...
    fn handle_update_manager_event(
            ...
            (&result.operation, &result.success_type)
        {
            ...
            if should_handle {
                ...
                self.show_success_toast(t!("settings.environments.updated_toast").to_string(), ctx);
                ...
                if let Some(result_client_id) = &result.client_id {
                    if *result_client_id == pending_client_id {
                        self.show_success_toast(
                            t!("settings.environments.created_toast").to_string(),
                            ctx,
                        );
                    }
                }
            ...
            if let Some(pending_env_id) = self.pending_delete_env_id.take() {
                ...
                if let Some(server_id) = &result.server_id {
                    if server_id.uid() == pending_env_id.uid() {
                        self.show_success_toast(
                            t!("settings.environments.deleted_toast").to_string(),
                            ctx,
                        );
                    }
                }
        ...
        if matches!(&result.operation, ObjectOperation::MoveToDrive) {
            ...
            if matches!(result.success_type, OperationSuccessType::Success) {
                self.show_success_toast(t!("settings.environments.shared_toast").to_string(), ctx);
            } else {
                self.show_error_toast(
                    t!("settings.environments.share_failed_toast").to_string(),
                    ctx,
                );
            }
        ...
        match event {
            UpdateEnvironmentFormEvent::Created {
                ...
                let Some(owner) = owner else {
                    self.show_error_toast(
                        t!("settings.environments.create_not_logged_in").to_string(),
                        ctx,
                    );
                    return;
                };
            ...
            UpdateEnvironmentFormEvent::Updated {
                ...
                let Some(existing_env) = CloudAmbientAgentEnvironment::get_by_id(env_id, ctx)
                    ...
                    self.show_error_toast(
                        t!("settings.environments.save_missing_environment").to_string(),
                        ctx,
                    );
                    return;
                };
        ...
        match action {
            ...
            EnvironmentsPageAction::ShareToTeam(env_id) => {
                let Some(team_uid) = UserWorkspaces::as_ref(ctx).current_team_uid() else {
                    self.show_error_toast(
                        t!("settings.environments.share_no_team").to_string(),
                        ctx,
                    );
                    return;
                };
                ...
                let SyncId::ServerId(server_id) = *env_id else {
                    self.show_error_toast(
                        t!("settings.environments.share_not_synced").to_string(),
                        ctx,
                    );
                    return;
                };
...
impl EnvironmentsPageWidget {
    ...
    fn render_no_matches_state(appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            Text::new(
                t!("settings.environments.no_matches").to_string(),
                appearance.ui_font_family(),
    ...
    fn render_scoped_section(
        ...
        let header = match list_scope {
            EnvironmentListScope::Personal => {
                Self::render_overline_header(&t!("settings.environments.personal"), appearance)
            }
            EnvironmentListScope::Team => {
                let shared_by_text = UserWorkspaces::as_ref(app)
                    .current_team()
                    .map(|team| {
                        t!("settings.environments.shared_by_team", team = team.name).to_string()
                    })
                    .unwrap_or_else(|| t!("settings.environments.shared_by_your_team").to_string());
    ...
    fn render_empty_state(
        ...
        let (github_button_label, github_button_enabled) = if dropdown_state.is_loading {
            (t!("settings.environments.loading").to_string(), false)
        } else if dropdown_state.load_error_message.is_some() {
            (t!("settings.environments.retry").to_string(), true)
        } else if dropdown_state.auth_url.is_some() {
            (t!("settings.environments.authorize").to_string(), true)
        } else {
            (t!("settings.environments.get_started").to_string(), true)
        };
        ...
        let local_repos_button = Self::render_empty_state_button(
            appearance,
            &t!("settings.environments.launch_agent"),
        ...
        let github_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Github,
                title: t!("settings.environments.quick_setup").to_string(),
                badge: Some(t!("settings.environments.suggested").to_string()),
                subtitle: t!("settings.environments.quick_setup_subtitle").to_string(),
        ...
        let local_repos_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Terminal,
                title: t!("settings.environments.use_agent").to_string(),
                badge: None,
                subtitle: t!("settings.environments.use_agent_subtitle").to_string(),
        ...
        let header = Text::new(
            t!("settings.environments.empty_header").to_string(),
            appearance.ui_font_family(),
        ...
        let subheader = Text::new(
            t!("settings.environments.empty_subheader").to_string(),
            appearance.ui_font_family(),
        ...
        Hoverable::new(card_hover_mouse_state, move |state| {
            ...
            let env_id_with_copy = render_copyable_text_field(
                CopyableTextFieldConfig::new(
                    t!("settings.environments.env_id", id = env_id_str.clone()).to_string(),
                )
            ...
            let mut details_parts =
                vec![t!("settings.environments.image", image = env_docker_image).to_string()];
            ...
            if !env_github_repos.is_empty() {
                let repos_text = env_github_repos
                    ...
                    .join(", ");
                details_parts
                    .push(t!("settings.environments.repos", repos = repos_text).to_string());
            }
            ...
            if !env_setup_commands.is_empty() {
                ...
                details_parts.push(
                    t!(
                        "settings.environments.setup_commands",
            ...
            let view_runs_link = appearance
                .ui_builder()
                .link(
                    t!("settings.environments.view_my_runs").to_string(),
                    None,
            ...
            if should_render_share_button {
                ...
                let share_button_element = if is_card_hovered {
                    ...
                    .with_tooltip(move || {
                        share_ui_builder
                            .tool_tip(t!("settings.environments.share").to_string())
                            .build()
                            .finish()
                    })
                    .build()
            ...
            if is_card_hovered {
                edit_button = edit_button.with_tooltip(move || {
                    edit_ui_builder
                        .tool_tip(t!("settings.environments.edit").to_string())
                        .build()
                        .finish()
                });
            }
...
impl BackingView for EnvironmentsPageView {
    ...
    fn render_header_content(
        ...
        HeaderContent::simple(t!("settings.sections.cloud_environments").to_string())
    }
