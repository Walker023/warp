...
impl BillingAndUsagePageV2View {
    ...
    fn handle_workspaces_event(
        ...
        match event {
            ...
            UserWorkspacesEvent::UpdateWorkspaceSettingsRejected(_err) => {
                ...
                self.show_toast(
                    &t!("settings.billing.failed_to_update_workspace_settings"),
                    ToastFlavor::Error,
                    ctx,
                );
            }
            ...
            UserWorkspacesEvent::PurchaseAddonCreditsSuccess => {
                ...
                self.show_toast(
                    &t!("settings.billing.successfully_purchased_add_on_credits"),
                    ToastFlavor::Success,
                    ctx,
                );
                        ...
                        Container::new(
                            appearance
                                ...
                                .with_text_and_icon_label(
                                    TextAndIcon::new(
                                        TextAndIconAlignment::IconFirst,
                                        t!("settings.teams.open_admin_panel").to_string(),
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &t!("settings.billing.base_credits"),
                        &reset_str,
                        base_remaining,
                        base_limit,
                        outline_color,
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        ...
                        &t!("settings.billing.personal_credits"),
            ...
            cards_row.add_child(
                Expanded::new(
                    ...
                    render_balance_card(
                        appearance,
                        BONUS_CREDITS_DOT_COLOR,
                        &t!("settings.billing.team_credits"),
        ...
        Some(
            Flex::column()
                .with_child(
                    Container::new(
                        Text::new_inline(
                            t!("settings.billing.balance").to_string(),
                            appearance.ui_font_family(),
                            HEADER_FONT_SIZE,
                        )
                        .with_style(Properties::default().weight(Weight::Bold))
    ...
    fn addon_credits_panel_state(
        ...
        let description_text = if team_count > 1 {
            format!(
                "{} {}",
                t!("settings.billing.addon_credits_description"),
                t!("settings.billing.addon_credits_personal_balance_description")
            )
        } else {
        ...
        let auto_reload_tooltip_text = t!(
            ...
            auto_reload_credit_amount = auto_reload_credit_amount
        )
        .to_string();
        let warning_text = if delinquent && has_admin_permissions {
            Some(t!("settings.billing.addon_credits_delinquent_warning").to_string())
        } else if delinquent {
            Some(t!("settings.billing.addon_credits_non_admin_delinquent_warning").to_string())
        } else if workspace
            .billing_metadata
            .has_failed_addon_credit_auto_reload_status()
        {
            Some(if has_admin_permissions {
                ...
                t!("settings.billing.restricted_billing_usage_non_admin_warning").to_string()
            })
        } else if would_exceed {
            Some(match (auto_reload_enabled, has_admin_permissions) {
                (true, true) => t!("settings.billing.auto_reload_paused_limit_admin").to_string(),
                (true, false) => {
                    t!("settings.billing.auto_reload_paused_limit_non_admin").to_string()
                }
                (false, true) => t!("settings.billing.purchase_exceeds_limit_admin").to_string(),
                (false, false) => {
                    t!("settings.billing.purchase_exceeds_limit_non_admin").to_string()
                }
            })
        } else {
            None
        ...
        if !has_admin_permissions && auto_reload_enabled {
            ...
            let description_text = match configured_auto_reload_option {
                Some(option) => {
                    ...
                    t!(
                        "settings.billing.non_admin_auto_reload_description_with_price",
                        credits = credits,
                        price = price
                    )
                    .to_string()
                }
                None => t!("settings.billing.non_admin_auto_reload_description").to_string(),
            };
    ...
    fn render_addon_credits_ineligible_plan_card(
        ...
        let explanation = match restriction {
            ...
            AddonCreditsRestriction::ContactTeamAdmin => appearance
                .ui_builder()
                .paragraph(t!("settings.billing.contact_team_admin_addon_credits").to_string())
                .with_style(UiComponentStyles {
                    font_color: Some(theme.sub_text_color(bg).into()),
                    ..Default::default()
                })
        ...
        let header = Text::new_inline(
            t!("settings.billing.buy_credits").to_string(),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
    ...
    fn render_addon_credits_non_admin_auto_reload_card(
        ...
        let auto_reload_header = Text::new_inline(
            t!("settings.billing.auto_reload_enabled").to_string(),
            appearance.ui_font_family(),
            HEADER_FONT_SIZE,
        )
        .with_color(theme.foreground().into())
    ...
    fn render_addon_credits_lower_section(
        ...
        let purchase_button_label = if self.addon_credits.purchase_loading {
            ...
            t!("settings.billing.one_time_purchase").to_string()
        };
            ...
            BillingAndUsagePageAction::UpdateAutoReloadEnabled { team_uid, enabled } => {
                let auto_reload_denomination_credits = if *enabled {
                    let Some(option) = self
                        ...
                        self.show_toast(
                            &t!("settings.billing.unable_enable_auto_reload_pricing"),
                            ToastFlavor::Error,
                            ctx,
                        );
                        return;
                ...
                self.pending_auto_reload_toast = Some(if *enabled {
                    ...
                    t!(
                        "settings.billing.auto_reload_enabled_toast",
                        credits = credits
                    )
                    .to_string()
                } else {
                    t!("settings.billing.auto_reload_disabled_toast").to_string()
                });
                UserWorkspaces::handle(ctx).update(ctx, |ws, ctx| {
                    ws.update_addon_credits_settings(
                        *team_uid,
