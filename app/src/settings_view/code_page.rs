...
impl CodeSubpage {
    ...
    pub fn title(&self) -> String {
        match self {
            Self::Indexing => t!("settings.code.codebase_indexing").to_string(),
            Self::EditorAndCodeReview => t!("settings.code.editor_and_code_review").to_string(),
        }
    }
}
...
impl CodeSettingsPageView {
    pub fn new(ctx: &mut ViewContext<CodeSettingsPageView>) -> Self {
        ...
        let manual_add_directory_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(t!("settings.code.index_new_folder"), SecondaryTheme)
        ...
        let page = if FeatureFlag::OpenWarpNewSettingsModes.is_enabled() {
            ...
            let categories = vec![
                Category::new(
                    t!("settings.code.codebase_indexing"),
                    codebase_indexing_widgets,
                ),
                Category::new(
                    t!("settings.code.code_editor_and_review"),
...
impl SettingsWidget for CodePageWidget {
    ...
    fn render(
        ...
        content.add_child(self.render_settings_subtext(
            global_ai_enabled,
            t!("settings.code.codebase_indexing_description").to_string(),
            appearance,
        ));
        content.add_child(self.render_settings_subtext(
            global_ai_enabled,
            t!("settings.code.warp_indexing_ignore_description").to_string(),
            appearance,
        ));
...
impl CodePageWidget {
    fn render_autoindexing_rows(
        ...
        let mut rows = vec![
            self.render_autoindex_row(
                t!("settings.code.index_new_folders_by_default").to_string(),
            ...
            self.render_settings_subtext(
                ...
                t!("settings.code.auto_index_description").to_string(),
                appearance,
            ),
        ];
        ...
        if codebase_indexing_enabled && !CodebaseIndexManager::as_ref(app).can_create_new_indices()
        {
            rows.push(self.render_settings_subtext(
                false,
                t!("settings.code.codebase_index_limit_reached").to_string(),
                appearance,
            ));
        }
    ...
    fn render_code_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            ui_builder
                .span(t!("settings.sections.code").to_string())
                .with_style(UiComponentStyles {
                    font_size: Some(24.0),
    ...
    fn render_initialization_settings_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            ui_builder
                .span(t!("settings.code.initialization_settings").to_string())
                .with_style(UiComponentStyles {
                    font_size: Some(18.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.active_ui_text_color().into()),
    ...
    fn render_codebase_indexing_toggle_row(
        ...
        let label = ui_builder
            .span(t!("settings.code.codebase_indexing_toggle").to_string())
            .with_style(UiComponentStyles {
                font_size: Some(16.0),
                font_weight: Some(Weight::Semibold),
                font_color: Some(theme.active_ui_text_color().into()),
        ...
        let disabled_tooltip_text = match admin_setting {
            AdminEnablementSetting::Enable => {
                Some(t!("settings.code.indexing_workspace_enabled_admin").to_string())
            }
            AdminEnablementSetting::Disable => {
                Some(t!("settings.code.indexing_disabled_admin").to_string())
            }
            AdminEnablementSetting::RespectUserSetting if !global_ai_enabled => {
                Some(t!("settings.code.indexing_disabled_global_ai").to_string())
            }
        ...
        content.add_child(
            Container::new(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .span(t!("settings.code.initialized_indexed_folders").to_string())
                            .with_style(UiComponentStyles {
                                font_size: Some(16.0),
                                font_weight: Some(Weight::Semibold),
                                font_color: Some(theme.active_ui_text_color().into()),
        ...
        if !rendered_folder {
            content.add_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .paragraph(t!("settings.code.no_initialized_folders").to_string())
                        .build()
                        .finish(),
                )
            ...
            let open_rules_button = ui_builder
                ...
                .with_text_and_icon_label(
                    warpui::ui_components::button::TextAndIcon::new(
                        ...
                        t!("settings.code.open_project_rules").to_string(),
    ...
    fn render_indexing_subsection_for_target(
        ...
        column.add_child(
            ui_builder
                .span(t!("settings.code.indexing_label").to_string())
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
    ...
    fn local_indexing_status_presentation(
        ...
        let Some(index_state) = index_state else {
            return IndexingStatusPresentation {
                text: t!("settings.code.index_status.no_index_created"),
        ...
        if index_state.has_pending() {
            let text = match index_state.sync_progress() {
                Some(SyncProgress::Discovering { total_nodes }) => Cow::from(
                    t!(
                        "settings.code.index_status.discovered_chunks",
                ...
                Some(SyncProgress::Syncing {
                    ...
                    t!(
                        "settings.code.index_status.syncing_progress",
                        completed_nodes = completed_nodes,
                        total_nodes = total_nodes
                    )
                    .to_string(),
                ),
                None => t!("settings.code.index_status.syncing"),
            };
        ...
        if let Some(completed_successfully) = index_state.last_sync_successful() {
            let (text, color, icon) = if completed_successfully {
                (
                    t!("settings.code.index_status.synced"),
                    theme.ansi_fg_green(),
                    Icon::Check,
                )
            ...
            {
                (
                    t!("settings.code.index_status.codebase_too_large"),
                    theme.ui_warning_color(),
                    Icon::AlertTriangle,
                )
            } else if index_state.has_synced_version() {
                (
                    t!("settings.code.index_status.stale"),
                    theme.nonactive_ui_detail().into_solid(),
                    Icon::ClockRefresh,
                )
            } else {
                (
                    t!("settings.code.index_status.failed"),
        ...
        IndexingStatusPresentation {
            text: t!("settings.code.index_status.no_index_built"),
    ...
    fn remote_indexing_status_presentation(
        ...
        match status.state {
            ...
            RemoteCodebaseIndexState::Unavailable => {
                ...
                IndexingStatusPresentation {
                    text: Cow::from(if limit_reached {
                        t!("settings.code.index_status.index_limit_reached").to_string()
                    } else {
                        t!("settings.code.index_status.unavailable").to_string()
                    }),
            ...
            RemoteCodebaseIndexState::Disabled => IndexingStatusPresentation {
                text: t!("settings.code.index_status.disabled"),
                color: theme.disabled_ui_text_color().into_solid(),
                icon: Some(Icon::SlashCircle),
                refresh_action: Some(IndexingRefreshAction::RequestRemote),
                show_delete: true,
            ...
            RemoteCodebaseIndexState::Queued => IndexingStatusPresentation {
                text: t!("settings.code.index_status.queued"),
                color: theme.disabled_ui_text_color().into_solid(),
                icon: None,
                refresh_action: None,
                show_delete: true,
            ...
            RemoteCodebaseIndexState::Indexing => {
                let text = match (status.progress_completed, status.progress_total) {
                    (Some(completed), Some(total)) => Cow::from(
                        t!(
                            "settings.code.index_status.indexing_progress",
                            completed = completed,
                            total = total
                        )
                        .to_string(),
                    ...
                    (Some(completed), None) => Cow::from(
                        t!(
                            "settings.code.index_status.indexing_completed",
                            completed = completed
                        )
                        .to_string(),
                    ),
                    (None, Some(total)) => Cow::from(
                        t!(
                            "settings.code.index_status.indexing_zero_total",
                            total = total
                        )
                        .to_string(),
                    ),
                    (None, None) => t!("settings.code.index_status.indexing"),
                };
            ...
            RemoteCodebaseIndexState::Ready => IndexingStatusPresentation {
                text: t!("settings.code.index_status.synced"),
            ...
            RemoteCodebaseIndexState::Stale => IndexingStatusPresentation {
                text: t!("settings.code.index_status.stale"),
            ...
            RemoteCodebaseIndexState::Failed => IndexingStatusPresentation {
                text: t!("settings.code.index_status.failed"),
    ...
    fn render_lsp_servers_subsection(
        ...
        content.add_child(
            ui_builder
                .span(t!("settings.code.lsp_servers_label").to_string())
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
    ...
    fn render_suggested_lsp_server_row(
        ...
        let (description, is_installing) = match &repo_status {
            Some(LspRepoStatus::DisabledAndInstalled { .. }) => {
                (t!("settings.code.lsp.installed").to_string(), false)
            }
            Some(LspRepoStatus::Installing { .. }) => {
                (t!("settings.code.lsp.installing").to_string(), true)
            }
            Some(LspRepoStatus::CheckingForInstallation) => {
                (t!("settings.code.lsp.checking").to_string(), true)
            }
            _ => (
                t!("settings.code.lsp.available_for_download").to_string(),
                false,
            ),
        };
        ...
        if is_failed {
            if let Some(server_handle) = server_model.cloned() {
                ...
                let restart_button = ui_builder
                    ...
                    .with_text_label(t!("settings.code.restart_server").to_string())
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            if has_logs {
                ...
                let view_logs_button = ui_builder
                    ...
                    .with_text_label(t!("settings.code.view_logs").to_string())
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
                ...
                match server.state() {
                    LspState::Available { .. } if !server.has_pending_tasks() => (
                        AnsiColorIdentifier::Green
                            ...
                            .into(),
                        t!("settings.code.lsp.available").to_string(),
                    ),
                    LspState::Starting | LspState::Available { .. } => (
                        AnsiColorIdentifier::Yellow
                            .to_ansi_color(&theme.terminal_colors().normal)
                            .into(),
                        t!("settings.code.lsp.busy").to_string(),
                    ),
                    LspState::Failed { .. } => (
                        AnsiColorIdentifier::Red
                            .to_ansi_color(&theme.terminal_colors().normal)
                            .into(),
                        t!("settings.code.lsp.failed").to_string(),
                    ),
                    LspState::Stopped { .. } | LspState::Stopping { .. } => (
                        theme.disabled_ui_text_color().into_solid(),
                        t!("settings.code.lsp.stopped").to_string(),
                    ),
                }
            }
            None => (
                theme.disabled_ui_text_color().into_solid(),
                t!("settings.code.lsp.not_running").to_string(),
            ),
        }
    }
}
...
impl SettingsWidget for CodeSubpageHeaderWidget {
    ...
    fn search_terms(&self) -> &str {
        "code codebase indexing editor code review"
    }
...
impl SettingsWidget for CodebaseIndexingCategorizedWidget {
    ...
    fn render(
        ...
        content.add_child(render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.codebase_indexing_toggle").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            toggle_element,
            Some(t!("settings.code.codebase_indexing_description").to_string()),
        ));
        ...
        if global_ai_enabled && codebase_context_enabled {
            ...
            content.add_child(render_body_item::<CodeSettingsPageAction>(
                ...
                ui_builder
                    ...
                    .finish(),
                Some(t!("settings.code.auto_index_description").to_string()),
            ));

            if !CodebaseIndexManager::as_ref(app).can_create_new_indices() {
                content.add_child(
                    ui_builder
                        .paragraph(t!("settings.code.codebase_index_limit_reached").to_string())
                        .with_style(UiComponentStyles {
...
impl SettingsWidget for AutoOpenCodeReviewPaneCodeWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.auto_open_code_review_panel").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.auto_open_code_review_panel_description").to_string()),
        )
    }
}
...
impl SettingsWidget for CodeReviewPanelToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.show_code_review_button").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.show_code_review_button_description").to_string()),
        )
    }
}
...
impl SettingsWidget for CodeReviewDiffStatsToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.show_diff_stats_on_code_review_button").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.show_diff_stats_on_code_review_button_description").to_string()),
        )
    }
}
...
impl SettingsWidget for ProjectExplorerToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.project_explorer").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.project_explorer_description").to_string()),
        )
    }
}
...
impl SettingsWidget for GlobalSearchToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.global_file_search").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.global_file_search_description").to_string()),
        )
    }
}
...
impl SettingsWidget for ShowHiddenFilesToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.show_hidden_files").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.show_hidden_files_description").to_string()),
        )
    }
}
...
impl SettingsWidget for FormatOnSaveToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            t!("settings.code.format_on_save").to_string(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(t!("settings.code.format_on_save_description").to_string()),
        )
    }
}
