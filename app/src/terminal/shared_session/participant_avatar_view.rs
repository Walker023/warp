...
impl ParticipantAvatarView {
    ...
    fn context_menu_items(&self) -> Vec<MenuItem<ParticipantAvatarAction>> {
        ...
        match self.role {
            Some(Role::Reader) => {
                items.extend([MenuItemFields::new(t!("terminal.make_editor").to_string())
            ...
            Some(Role::Executor) => {
                items.extend([MenuItemFields::new(t!("terminal.make_viewer").to_string())
                    .with_on_select_action(ParticipantAvatarAction::UpdateRole {
                        participant_id,
                        role: Role::Reader,
                    })
...
pub fn render_revoke_all_button(
    ...
    Hoverable::new(mouse_state_handle, |state| {
        ...
        if state.is_hovered() {
            ...
            stack.add_positioned_child(
                render_tooltip(
                    t!("terminal_ui.shared_session.roles.revoke_all_edit").to_string(),
                    appearance,
                ),
                OffsetPositioning::offset_from_parent(
                    vec2f(0., 3.),
...
pub fn render_viewer_role_button(
    ...
    let button = icon_button(appearance, icon, false, mouse_state_handle.clone())
        .with_tooltip(move || {
            ui_builder
                .tool_tip(t!("terminal.change_role").to_string())
                .build()
                .finish()
        })
        .build()
