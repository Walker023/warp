...
impl Body {
    pub fn open(
        ...
        let scrollback_from_active_block_message = if model.is_alt_screen_active() {
            t!("terminal_ui.shared_session.share.current_screen").to_string()
        } else if model
            .block_list()
            .active_block()
            .is_active_and_long_running()
        {
            t!("terminal_ui.shared_session.share.current_block").to_string()
        } else {
            t!("terminal_ui.shared_session.share.without_scrollback").to_string()
        };
        ...
        let mut options = vec![
            ...
            ScrollbackOption {
                label: t!("terminal_ui.shared_session.share.session_start").to_string(),
        ...
        } = open_source
        {
            ...
            options.insert(
                0,
                ScrollbackOption {
                    label: t!("terminal_ui.shared_session.share.selected_block").to_string(),
                    scrollback_type,
                    mouse_state_handle: Default::default(),
                    is_disabled,
                },
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let mut start_sharing_button = appearance
            ...
            .with_centered_text_label(
                t!("terminal_ui.shared_session.share.start_sharing").to_string(),
            )
        ...
        let explanation_message = if disabled_count == 0 {
            ...
            if self.has_agent_conversations {
                Some(t!("terminal_ui.shared_session.share.disabled_size_and_agents").to_string())
            } else {
                Some(t!("terminal_ui.shared_session.share.disabled_size").to_string())
            }
        } else {
            // Only one option disabled - use specific message if it's due to agent conversations
            if self.has_agent_conversations {
                Some(
                    t!("terminal_ui.shared_session.share.disabled_agent_conversations").to_string(),
                )
            } else {
                Some(t!("terminal_ui.shared_session.share.disabled_size").to_string())
            }
