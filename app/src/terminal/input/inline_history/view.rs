...
fn build_tab_configs(is_agent_view: bool) -> Vec<InlineMenuTabConfig<HistoryTab>> {
    ...
    vec![
        ...
        InlineMenuTabConfig {
            ...
            label: t!("terminal_ui.input.history.commands").to_string(),
            filters: HashSet::from([QueryFilter::Commands]),
        },
        InlineMenuTabConfig {
            id: HistoryTab::Prompts,
            label: t!("terminal_ui.input.history.prompts").to_string(),
    ...
    fn new_inner(
        ...
        let menu_view = if FeatureFlag::InlineMenuHeaders.is_enabled() {
            let configure_button = ctx.add_view(|_| {
                ActionButton::new(t!("terminal.configure").to_string(), ConfigureButtonTheme)
            ...
            let header_config = InlineMenuHeaderConfig {
                label: t!("terminal_ui.input.history.title").to_string(),
