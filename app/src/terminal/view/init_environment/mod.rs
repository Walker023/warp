    ...
    fn render_pending_step(
        ...
        if self.repos.is_empty() && !self.use_current_dir {
            let help_text = Text::new(
                t!("terminal_ui.init_environment.no_repos_help").to_string(),
        ...
        RenderableAction::new_with_element(content.finish(), app)
            .with_header(
                HeaderConfig::new(t!("terminal_ui.init_environment.explanation"), app)
...
impl View for InitEnvironmentBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let rendered_step = match &self.setup_state {
            ...
            SetupState::Skipped => {
                RenderableAction::new(t!("terminal_ui.init_environment.cancelled").as_ref(), app)
